    ...
    if len(commands) == 1:
        ...
        return await msg.reply(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("Aç", callback_data="on", style=ButtonStyle.SUCCESS),
                        InlineKeyboardButton("Kapat", callback_data="off", style=ButtonStyle.DANGER),
                    ]
                ]
            ),
        )
...
async def chatModeHandler(bot: app, msg: Message):
    ...
    if text.startswith("fizy"): 
        reply = random.choice(fizy)
